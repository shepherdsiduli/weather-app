...
import za.co.shepherd.weatherapp.databinding.FragmentSearchBinding
...
import za.co.shepherd.weatherapp.ui.search.response.SearchResponseAdapter
import za.co.shepherd.weatherapp.utils.extensions.hideKeyboard
...
class SearchFragment : BaseFragment<SearchViewModel, FragmentSearchBinding>(
    ...
    private fun initSearchResultsAdapter() {
        val adapter = SearchResponseAdapter { item ->
            item.coordinates?.let {
                binding.viewModel?.saveCoordinatesToSharedPref(it)
                    ?.subscribe { _, _ ->
    ...
    private fun initSearchResultsRecyclerView(list: List<CitiesForSearchEntity>) {
        (binding.recyclerViewSearchResults.adapter as SearchResponseAdapter).submitList(
