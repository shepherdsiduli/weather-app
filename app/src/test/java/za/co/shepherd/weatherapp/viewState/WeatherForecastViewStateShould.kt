package za.co.shepherd.weatherapp.viewState

import android.os.Build
import androidx.test.ext.junit.runners.AndroidJUnit4
import com.google.common.truth.Truth
import org.junit.Test
import org.junit.runner.RunWith
import org.robolectric.annotation.Config
import za.co.shepherd.weatherapp.ui.dashboard.WeatherForecastViewState
import za.co.shepherd.weatherapp.utils.domain.Status

@Config(sdk = [Build.VERSION_CODES.P])
@RunWith(AndroidJUnit4::class)
class WeatherForecastViewStateShould {
    @Test
    fun `return loading true when status is loading`() {
        // Given
        val givenViewState = WeatherForecastViewState(status = Status.LOADING)

        // When
        val actualResult = givenViewState.isLoading()

        // Then
        Truth.assertThat(actualResult).isTrue()
    }

    @Test
    fun `not return loading false when status is error`() {
        // Given
        val givenViewState = WeatherForecastViewState(status = Status.SUCCESS)

        // When
        val actualResult = givenViewState.isLoading()

        // Then
        Truth.assertThat(actualResult).isFalse()
    }

    @Test
    fun `not return loading false when status is success`() {
        // Given
        val givenViewState = WeatherForecastViewState(status = Status.ERROR)

        // When
        val actualResult = givenViewState.isLoading()

        // Then
        Truth.assertThat(actualResult).isFalse()
    }

    @Test
    fun `return correct error message when status is error`() {
        // Given
        val givenViewState =
            WeatherForecastViewState(
                status = Status.ERROR,
                error = "500 Internal Server Error"
            )

        // When
        val actualResult = givenViewState.getErrorMessage()

        // Then
        Truth.assertThat(actualResult).isEqualTo("500 Internal Server Error")
    }

    @Test
    fun `return true for error placeholder visibility  when status is error`() {
        // Given
        val givenViewState =
            WeatherForecastViewState(
                status = Status.ERROR,
                error = "500 Internal Server Error"
            )

        // When
        val actualResult = givenViewState.shouldShowErrorMessage()

        // Then
        Truth.assertThat(actualResult).isTrue()
    }
}